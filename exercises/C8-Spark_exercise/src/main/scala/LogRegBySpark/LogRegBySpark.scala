object LogRegBySpark {
  ...
  def readData(fileName: String, sep: String, header: Boolean = true, inferSchema: Boolean = true) = {
  ...
  def logisticRegression(standardization: Boolean = false, elasticNetParam: Double = 0.0, regParam: Double = 0.1, maxIter: Int = 100): LogisticRegression  = {
    import org.apache.spark.ml.classification._
    import org.apache.spark.ml.classification.{LogisticRegression, LogisticRegressionModel}
  ...
  def searchBestHyperParam(logReg: LogisticRegression, dflogr: DataFrame, nFold: Int = 3): Tuple4[CrossValidatorModel, Double, LogisticRegression, LogisticRegressionModel]= {
  ...
  def main(args: Array[String]): Unit = {
    val (spark, sc, df) = readData("data/spambase.data", sep = ",", header = false, inferSchema = true)
    ...
    println( s"#features whose coeeficient is equal to 0.0: ${nZeros}")
